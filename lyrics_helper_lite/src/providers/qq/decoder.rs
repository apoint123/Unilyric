...
pub fn decrypt_qrc(encrypted_text: &str) -> Result<String> {
    let encrypted_bytes = hex::decode(encrypted_text)
        .map_err(|e| crate::error::FetcherError::Provider(format!("无效的十六进制字符串: {e}")))?;
...
mod qrc_logic {
    ...
    pub(super) fn decrypt_lyrics_from_bytes(encrypted_bytes: &[u8]) -> Result<String> {
        ...
        String::from_utf8(decompressed_bytes)
            .map_err(|e| FetcherError::Provider(format!("UTF-8编码转换失败: {e}")))
    }
    ...
    fn decompress(data: &[u8]) -> Result<Vec<u8>> {
        ...
        decoder
            ...
            .map_err(|e| FetcherError::Provider(format!("Zlib解压缩失败: {e}")))?;
