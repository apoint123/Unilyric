...
fn handle_progress_update(new_info: &smtc_suite::NowPlayingInfo, state: &ActorState) -> bool {
        ...
        (&state.websocket_client, &state.websocket_status)
    {
        ...
        let duration_was_missing = last_info.is_none_or(|i| i.duration_ms.unwrap_or(0) == 0);
        let duration_is_now_present = new_info.duration_ms.unwrap_or(0) > 0;

        if duration_was_missing && duration_is_now_present {
            send_music_info_to_ws(client, new_info);
        }
